## [Version: v3.15.2-alpha3] - 2026-09-15
**缺陷修复**
- 修复多租户版本推送主机身份接口报错的问题

## [Version: v3.15.2-alpha2] - 2026-07-27
**功能优化**
- 更新helm chart的bitnami仓库地址

**缺陷修复**
- 修复导入模型字段报错的问题
- 集群与服务模板“组织”字段编辑交互问题修复
- 修复同步集群模板实例后没有显示同步人的问题
- 修复鼠标hover溢出的用户名没有tips完整显示用户信息的问题
- 修复主机全部列表翻页后切换已分配未分配无数据的问题
- 修复模型表格字段默认值操作按钮被遮挡的问题
- 修复IPv6主机操作审计详情实例没有显示的问题
- 修复hover用户和退出文案底部颜色与需求不一致的问题
- 修复个人中心文案顺序图标与需求不一致的问题

## [Version: v3.15.2-alpha1] - 2026-07-16
**新增功能**
- 登录人员信息展示使用统一的组件并添加企业空间信息
- 添加bk-ops专用的查询业务集下的容器信息的特殊接口
- 支持BCS同步CRD容器数据
- 添加 cw_aitsm 的 apigw 默认权限
- 支持前端隐藏模型属性

**功能优化**
- 主机模型CPU架构字段添加常见类型
- 基础镜像更新为TencentOS4
- 校验mongodb条件不能包含危险操作符
- pod表添加业务ID索引

**缺陷修复**
- 业务拓扑中自定义字段bool字段默认值无效
- 回滚apigw文档为swagger格式并修复导入模型bool解析错误的问题
- xss过滤补丁
- 导出Excel xlsx表格DDE问题
- 字段组合模板-移除绑定的模型后鉴权错误
- 集群模板编辑“组织”字段不生效问题
- 集群/模块节点信息编辑修改时空值与新建时候不匹配
- 修复通过日期字段查询模型实例失败的问题

## [Version: v3.15.1] - 2026-03-24
**新增功能**
- 支持多租户架构，可通过配置开启或关闭多租户模式；开启时支持系统租户（system）和普通租户，不开启时默认使用单一default租户
- 新增租户集管理功能，支持查询租户集列表、查询租户集中的租户、查询租户集中的业务及主机等
- 支持按租户维度进行MongoDB数据库分片存储，实现租户间数据隔离
- 支持租户维度的全局配置管理，每个租户拥有独立的全局配置
- 支持蓝鲸时区优化方案，模型下的时间字段按用户时区格式展示，导出excel文件支持转换用户时区
- 支持运维需求，新增修改业务时区的能力
- 接入审计中心，支持操作审计数据上报到蓝鲸审计中心
- 字段类型支持插件扩展机制
- 添加前端版本更新检查并提示刷新功能
- 支持页面权限中心及用户管理入口
- 前端支持子路径浏览图标
- 提供从非多租户版本升级数据到多租户版本的迁移工具

**功能优化**
- authServer多租户模式适配，支持按租户进行权限管理
- 单租户模式下ESB接口适配优化，兼容非多租户部署场景
- 优化APIGW接口文档，新增租户集相关接口文档
...
## [Version: v3.14.8-beta1] - 2026-05-22
**新增功能**
- 添加bk-ops专用的查询业务集下的容器信息的特殊接口
...
**缺陷修复**
- 修复通过日期字段查询模型实例失败的问题
- 字段组合模板移除绑定的模型后鉴权与集群模板编辑“组织”字段问题修复
- 集群/模块节点信息编辑修改时空值与新建时候不匹配问题修复
- 升级依赖处理安全问题
- 修复导入模型bool解析错误的问题

## [Version: v3.14.7] - 2026-04-15
...
**缺陷修复**
- 修复前端业务集主机批量复制"管控区域ID:IP"复制出无效信息、主机操作系统类型统计指标图例取值错误、模型默认停用后仍在收藏导航栏显示等问题
- 修复前端业务拓扑新增主机到模块搜索框模型count不全、表单创建重复提交、编辑直接新建集群/模块下自定义字段显示为模板创建且跳转报错、导出提示等问题
- 修复前端字段模版同步唯一校验未获取最新数据、编辑主机自动应用取消属性字段后下一步按钮置灰、pod详情点击所属拓扑报错等问题
- 修复主机自动应用查询问题及当前状态搜索条件失效问题
- 修复动态分组搜索分隔符优化、日期类型条件出现重复项、条件类型保存错误、主机条件in或not in时查询错误等相关问题
- 修复导出xlsx表格DDE问题及拓扑报错提示信息问题
- 修复服务实例添加主机时使用IPv6搜索报错及业务拓扑中自定义字段bool字段默认值无效问题
- 修复创建模型attrs时空指针报错及模型间存在多条关联场景关联查询失败问题
- 修复前端运营分析下资源查询报错及创建进程模版空格校验问题
- 修复枚举类型字段默认值选择异常问题及采集主机数据中IP随机排序问题
- 修复切换业务集时参数错误提示及业务集新建后操作无权限无法展示资源名称问题
- 添加资源主机转移操作预鉴权
- 修复新增主机到模块选择主机后点击下一步页面显示空白问题
- 修复项目列表多出一个项目图标字段问题
- 修复集群模板英文状态下更新模版属性后同步功能不支持点击跳转问题
- 修复服务实例批量编辑标签时没有及时触发校验规则问题
- 修复主机/实例高级搜索操作符为contains或contains(cs)时输入\报错问题
- 修复启停模型操作审计日志显示操作类型错误问题
- 修复运行统计实例统计时统计数据错误问题
...
**缺陷修复**
- 修复主机自动应用条件查询问题
- 修复启停模型操作审计日志显示操作类型错误
- 修复编辑直接新建的集群/模块下的自定义字段会显示是通过模板创建的，点击跳转对应模板会报错问题
- 修复表单创建重复提交问题
- 修复字段模版同步唯一校验没有获取最新数据问题
- 修复pod详情下点击所属拓扑报错问题
- 修复编辑主机自动应用时取消已选择的属性字段，‘下一步’按钮变成置灰了不能点击问题
